...
**Project:** modelstack.digital, agor.me, iOS apps, scored.tools (see `briefs/`). Read the relevant brief before suggesting campaigns. Email drafts use Gmail MCP (`mcp__claude_ai_Gmail__create_draft` for copy, never send). Automation via Windows Task Scheduler (S4U logon type, no Interactive); outbound actions draft-gated only. Analytics: Plausible (live) and Stripe MCP read-only (shared account acct_1T09QrAOqOwPWk86). See `data/REMAP.md` for full tool map.
