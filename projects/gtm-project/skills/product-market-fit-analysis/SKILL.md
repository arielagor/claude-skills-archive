...
**Project:** modelstack.digital (Stripe Payment Links, download metrics via Stripe MCP), agor.me (consult bookings via Google Calendar, `mcp__gbrain__query` for past engagement), iOS apps (App Store Connect analytics, IAP metrics). Read the relevant `briefs/<property>.md` before assessing PMF. Retention data from Plausible (web) or app analytics. Reference customers sourced from GBrain past interactions + testimonials.
