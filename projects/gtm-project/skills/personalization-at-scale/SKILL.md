...
**Research Sources**:
- GBrain semantic query (`mcp__gbrain__query`) for prior context with prospect or company
- GBrain backlinks (`get_backlinks`) to surface all past conversations, emails, notes linked to prospect
- WebSearch for company news, press releases, funding announcements
- LinkedIn activity (posts, comments, job changes) via manual review or WebSearch
- Product launches and updates (from company site, announcement search)
- Hiring patterns (job postings, LinkedIn careers page)
- Conference attendance/speaking (company blog, news)
- Blog posts and thought leadership
- Mutual connections (GBrain graph traversal)
...
**Export Formats**:
- CSV with personalization columns (for manual routing to email)
- Gmail draft (via `mcp__claude_ai_Gmail__create_draft`, subject + body per prospect)
- Individual email templates (copy-paste text blocks)
- Markdown summary for Ariel's review

**Recommended Workflow**:
1. Generate personalizations + confidence scores
2. Create Gmail drafts for top prospects
3. Ariel reviews drafts and sends manually from Gmail
4. Log outcome (response, no response, bounced) back to GBrain via `add_timeline_entry`
5. Track response rates by personalization type + confidence
